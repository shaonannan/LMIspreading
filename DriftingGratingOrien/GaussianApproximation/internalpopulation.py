...
class RecurrentPopulation(object):
    ...
    def update_rhoEQ(self):
        ...
        Vs,D,V = vs.copy(), ds.copy(), vedges.copy()
        Rv = np.copy(V)
        (vT,vR) = (1.0,0.0)
        ...
        try:
            Rv[indp[0][:]] = np.exp(-np.square(V[indp[0][:]]-Vs)/D)#np.exp(-(V[indp[0][:]]-Vs)**2/D)
            
            if(indp[0][0]>1):
                Rv[0:indp[0][0]] = np.exp(-np.square(V[0:indp[0][0]]-Vs)/D)
...
#                        VI = np.append(VI,c.V_sample_post)
    ...
    def update(self):
        ...
        if self.USUALorMFE:
            ...
            firing_rate  = -gL*ds/2.0*rhov[-1]*(-(1.0-vs)/ds)
